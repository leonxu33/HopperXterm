// Vendored verbatim from github.com/jlaffaye/ftp@v0.2.0 (parse.go),
// repackaged as ftpc. This is the proven, quirk-tolerant LIST-output
// parser (RFC3659 / Unix ls / MS-DOS DIR / hostedftp). Entry / EntryType
// and the setSize/setTime helpers it relies on live in client.go.
package ftpc
...
// ParseUnixListLine exposes the internal LIST-line parser to sibling
// transports. The SCP transport, which has no native directory listing,
// feeds it `ls -la` output line by line — FTP LIST output is the same
// `ls -l` format, so this reuses the proven, quirk-tolerant parser
// instead of hand-rolling a fragile one. Returns errUnsupportedListLine
// for non-entry lines (e.g. the `total N` header) so callers can skip them.
func ParseUnixListLine(line string, now time.Time, loc *time.Location) (*Entry, error) {
	return parseListLine(line, now, loc)
}
...
func (e *Entry) setTime(fields []string, now time.Time, loc *time.Location) (err error) {
	if strings.Contains(fields[2], ":") { // contains time
		...
		// months old. If the parsed time ends up in the future, it really
